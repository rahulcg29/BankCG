...
try:
    ...
    st.error("Error: cgbank_data.json file not found!")
    st.stop()
except json.JSONDecodeError:
    st.error("Error: Invalid JSON format in cgbank_data.json!")
    st.stop()
...
for key in REQUIRED_KEYS:
    if key not in BANK_DATA:
        if key == 'account_requests':
            ...
            st.error(f"Error: Missing required key '{key}' in cgbank_data.json!")
            st.stop()
